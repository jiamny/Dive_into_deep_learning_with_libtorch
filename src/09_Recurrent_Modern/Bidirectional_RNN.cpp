...
struct RNNModelbirnn : public torch::nn::Module {
    //"""The deep rnn model."""
	...
	RNNModelbirnn( torch::nn::LSTM rnn_layer, int64_t vocab_size ) {
        rnn = rnn_layer;
...
int main() {
	...
	torch::nn::LSTM lstm_layer(torch::nn::LSTMOptions(vocab.length(), num_hiddens).num_layers(num_layers).bidirectional(true));

	auto net = RNNModelbirnn(lstm_layer, vocab.length());
	net.to(device);
	...
	torch::nn::LSTM clstm_layer(torch::nn::LSTMOptions(vocab.length(), num_hiddens).num_layers(num_layers).bidirectional(true));
	RNNModelbirnn cnet(clstm_layer, vocab.length());
	cnet.to(device);
	...
	auto ax1 = F->nexttile();
	matplot::plot(ax1, ctrlt.first, ctrlt.second, "b")->line_width(2);
    matplot::xlabel(ax1, "epoch");
    matplot::ylabel(ax1, "perplexity");
    matplot::title(ax1, "RNNModelbirnn");
    matplot::show();
